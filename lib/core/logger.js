...
function rotateLogIfNeeded(logPath) {
  ...
  try {
    ...
    console.warn(`[FORV WARN] Failed to rotate log ${logPath}: ${error.message}`);
  }
}
...
function createLogStreams(appName) {
  ...
  try {
    ...
    outStream.on('error', (error) => {
      console.error(`[FORV ERROR] Failed to write to ${outLog}: ${error.message}`);
    });

    errStream.on('error', (error) => {
      console.error(`[FORV ERROR] Failed to write to ${errLog}: ${error.message}`);
    });
...
function getLastLines(filePath, lines = 50) {
  ...
  try {
    ...
    console.warn(`[FORV WARN] Failed to read log file ${filePath}: ${error.message}`);
    return '';
  }
}
  ...
  [outLog, errLog, `${outLog}.1`, `${errLog}.1`].forEach(logPath => {
    if (fs.existsSync(logPath)) {
      try {
        ...
        console.warn(`[FORV WARN] Failed to delete log ${logPath}: ${error.message}`);
      }
    }
  });
}
