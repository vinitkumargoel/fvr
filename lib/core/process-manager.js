...
function startForkMode(appConfig, state, onExit) {
  ...
  // Determine if script is a system command or a file
  const isSystemCommand = ['npm', 'yarn', 'pnpm', 'bun', 'deno', 'python', 'python3', 'ruby', 'php'].includes(script);
  const isNodeScript = script.endsWith('.js') || script.endsWith('.cjs') || script.endsWith('.mjs') || script.startsWith('./') || script.startsWith('../');

  let command, commandArgs;

  if (isSystemCommand) {
    // Run system command directly (npm, yarn, etc.)
    command = script;
    commandArgs = args;
  } else if (isNodeScript || script === 'node') {
    // Run Node.js script
    if (script === 'node') {
      // If script is 'node', first arg is the actual script
    ...
    // Assume it's a Node.js script without extension
    command = process.execPath;
    commandArgs = [script, ...args];
  }
...
function startClusterMode(appConfig, state, onExit) {
  ...
  // Check if script is a system command - cluster mode only works with Node.js scripts
  const isSystemCommand = ['npm', 'yarn', 'pnpm', 'bun', 'deno', 'python', 'python3', 'ruby', 'php'].includes(script);

  if (isSystemCommand) {
    throw new ProcessError(
      `Cluster mode is not supported for system commands like '${script}'. ` +
      `Use fork mode instead (remove --instances or set --instances 1).`
    );
  }
