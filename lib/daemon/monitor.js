...
function setupAutoRestart(app, restartCallback, updateAppState) {
  ...
  monitor.restartCallback = async (exitInfo) => {
    console.log(`[FVR] Process ${app.name} exited with code ${exitInfo.code}`);

    // Check if autorestart is enabled
    if (!app.autorestart) {
      console.log(`[FVR] Auto-restart disabled for ${app.name}`);
    ...
    if (checkCrashLoop(updatedApp)) {
      console.error(`[FVR ERROR] App ${app.name} in crash loop (${CRASH_LOOP.maxRestarts} restarts in ${CRASH_LOOP.timeWindow / 1000}s). Stopping auto-restart.`);
    ...
    console.log(`[FVR] Restarting ${app.name} in ${TIMEOUTS.restartDelay / 1000}s...`);
    setTimeout(async () => {
      try {
        await restartCallback(app);
        console.log(`[FVR] App ${app.name} restarted successfully`);
      } catch (error) {
...
function startMemoryMonitoring(app, onMemoryExceeded) {
  ...
  if (!memoryLimit) {
    console.warn(`[FVR WARN] Invalid memory limit for ${app.name}: ${app.max_memory_restart}`);
    return;
  }
  ...
  monitor.memoryInterval = setInterval(() => {
    ...
    if (totalMemory > memoryLimit) {
      ...
      console.warn(`[FVR WARN] App ${app.name} exceeded memory limit: ${memoryMB}MB > ${limitMB}MB. Restarting...`);
...
function startWatcher(app, onFileChange) {
  ...
  const handleChange = (path) => {
    console.log(`[FVR] File changed in ${app.name}: ${path}`);
    ...
    restartTimeout = setTimeout(() => {
      console.log(`[FVR] Restarting ${app.name} due to file change...`);
      if (onFileChange) {
  ...
  watcher.on('error', (error) => {
    console.error(`[FVR ERROR] Watcher error for ${app.name}: ${error.message}`);
  });

  monitor.watcher = watcher;
  monitors.set(app.name, monitor);

  console.log(`[FVR] Watching files for ${app.name} in ${app.cwd}`);
}
...
function stopWatcher(appName) {
  ...
  if (monitor?.watcher) {
    ...
    console.log(`[FVR] Stopped watching files for ${appName}`);
  }
}
