...
async function updateAppState(appName, updates) {
  try {
    ...
    console.error(`[FVR ERROR] Failed to update app state: ${error.message}`);
  }
}
...
async function handleMemoryExceeded(app) {
  try {
    ...
    console.error(`[FVR ERROR] Failed to restart app after memory exceeded: ${error.message}`);
  }
}
...
async function handleFileChange(app, filePath) {
  try {
    ...
    console.log(`[FVR] App ${app.name} restarted due to file change`);
  } catch (error) {
    console.error(`[FVR ERROR] Failed to restart app after file change: ${error.message}`);
  }
}
...
async function handleCommand(command, args) {
  try {
    ...
    console.error(`[FVR ERROR] Command failed: ${error.message}`);
    throw error;
  }
}
...
function startDaemon() {
  console.log('[FVR] Starting daemon...');
  ...
  console.log(`[FVR] Daemon started (PID: ${process.pid})`);

  // Setup signal handlers
  process.on('SIGTERM', () => {
    console.log('[FVR] Received SIGTERM, shutting down...');
    shutdown();
  });

  process.on('SIGINT', () => {
    console.log('[FVR] Received SIGINT, shutting down...');
    shutdown();
  });

  // Handle uncaught errors
  process.on('uncaughtException', (error) => {
    console.error('[FVR ERROR] Uncaught exception:', error);
    // Don't exit on uncaught exceptions, daemon should stay alive
  });

  process.on('unhandledRejection', (reason) => {
    console.error('[FVR ERROR] Unhandled rejection:', reason);
...
async function shutdown() {
  ...
  isShuttingDown = true;
  console.log('[FVR] Shutting down daemon...');
  ...
  try {
    ...
    console.log('[FVR] Daemon shut down successfully');
    process.exit(0);
  } catch (error) {
    console.error('[FVR ERROR] Error during shutdown:', error);
    process.exit(1);
  }
}
