...
function createIPCServer(commandHandler) {
  ...
  if (fs.existsSync(DAEMON_SOCK)) {
    try {
      ...
      console.warn(`[FVR WARN] Failed to remove existing socket: ${error.message}`);
    }
  }
    ...
    socket.on('data', async (data) => {
      ...
      for (const message of messages) {
        ...
        try {
          ...
          console.error(`[FVR ERROR] Failed to parse IPC message: ${error.message}`);
        }
      }
    });

    socket.on('error', (error) => {
      console.error(`[FVR ERROR] Socket error: ${error.message}`);
    });
  ...
  server.listen(DAEMON_SOCK, () => {
    ...
    try {
      ...
      console.warn(`[FVR WARN] Failed to set socket permissions: ${error.message}`);
    }
  });
    ...
    server.close(() => {
      ...
      if (fs.existsSync(DAEMON_SOCK)) {
        try {
          ...
          console.warn(`[FVR WARN] Failed to remove socket file: ${error.message}`);
        }
      }
      resolve();
    });
